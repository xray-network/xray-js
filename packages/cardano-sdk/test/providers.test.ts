...
describe("Provider", async () => {
  ...
  it("getUtxosByAddresses()", async () => {
    ...
    assert.ok(utxos instanceof Array)
  })
  ...
  it("getDatumByHash()", async () => {
    ...
    assert.deepEqual(datum, datumHex)
  })
  ...
  it("getScriptByHash()", async () => {
    ...
    assert.ok("script" in Object(script))
  })
})

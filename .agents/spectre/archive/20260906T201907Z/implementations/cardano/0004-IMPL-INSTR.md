...
| `.prettierrc`                             | `LOCAL` | Yes      | Repository formatting policy applied to changed Cardano source and tests.                                                       |
| `.prettierignore`                         | `LOCAL` | Yes      | Formatting exclusions for generated output, dependencies, vendored code, and XRAY evidence.                                     |
| `package.json`                            | `LOCAL` | Yes      | Repository formatting commands used to write and verify owned files consistently.                                               |
...
| C05       | Remove stale comments, redundant `async`/`await`, repeated object assembly, and non-domain aliases exposed only to work around the old layout. Keep comments that explain Cardano semantics or non-obvious low-level library constraints. Apply the repository Prettier policy, exclude generated/dependency/vendor/XRAY paths, and provide write/check commands.                                                          | No public symbol or documented import is removed; generated declarations remain compatible at the package entrypoints.                                                                                      | Repository formatting, changed Cardano modules, tests  | Prettier write/check, public API searches, package/repository validation, and diff review.                                                       |
